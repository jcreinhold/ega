...
and the citation forms for `(I, …)`, `(II, …)`, `(III, …)`, `(0, …)`, `(0_III, …)`, `(M, …)`, `(G, …)`, `(T, …)`,
`(FAC, …)`. Re-read those sections in `docs/books/ega/iii/conventions.md` before extending the ledger or making a
stylistic choice here.

**Block labels.** English: `**Proposition (1.1.4).**`, `**Definition (1.2.1).**`, `**Corollary (1.2.3).**`,
`**Theorem (1.8.4) (Chevalley).**`, `**Lemma (3.2.1).**`, `**Remark (1.6.4).**`, `**Scholium (2.3.3).**`. Never French
...
- `(0_IV, 19.3.5)` cites a paragraph in `06-ch0-19-formally-smooth-algebras.md` (Chap 0_IV §19).
- `(IV, 17.6.2)` cites EGA IV itself (used by later EGA volumes and by cross-section references inside EGA IV).
- `(0_III, 13.4.2)` cites Chap 0_III in EGA III.
...
The Nagata text *Local Rings* (Interscience, 1962) is cited as `(Nagata, …)` with chapter and section number. Zariski-
Samuel *Commutative Algebra* (Van Nostrand, 1958-1960) is cited as `(Zariski-Samuel, …)` with volume, chapter, section.
...
- Module of relative differentials (rings): `Ω_{B/A}`. EGA writes `Ω^1_{B/A}`; we render as `Ω_{B/A}` when no higher
    differentials are in play and as `Ω^1_{B/A}` when the source needs the index for disambiguation.
- Higher exterior powers: `Ω^p_{B/A} = Λ^p Ω_{B/A}`.
- Module of relative differentials (schemes): `Ω^1_{X/Y}` (or `Ω_{X/Y}` when no index is needed); locally on
    `Spec(B)   → Spec(A)` this is `Ω^1_{B/A}` glued.
- Universal derivation: `d : B → Ω_{B/A}`, `d_{B/A}` when the source/target needs naming.
- Derivation modules: `Der_A(B, M)`, `Der(B, M)` (when `A` is understood); EGA's `T_{B/A}(M)` notation is preserved when
    used.
- Frobenius: `F : A → A`, `a ↦ a^p` (characteristic `p > 0`).
- `p`-basis: a family `(b_i)` whose images in `A/A^p` form a basis (EGA's "`p`-base"). We render `p`-base → "`p`-basis".
...
EGA IV §0_IV.19 and §IV.17 introduce the family of "formally `P`" properties (`P ∈ {smooth, étale, unramified}`). We fix
...
Where EGA writes "formellement lisse pour la topologie `𝒥`-préadique", we render "formally smooth for the `𝒥`-preadic
topology". The qualifier ("for the discrete topology", "for the `J`-adic topology", etc.) is kept; do not silently drop
it. EGA's stylistic device of inserting the topology as a parenthetical adverb is preserved when it occurs.
...
- Local dimension at a point: `dim_x(X)`.
- Codimension of `Y` in `X` at a point `y`: `codim_y(Y, X)`.
- Depth (`profondeur`): `prof(M)`, `prof_𝔪(M)`. We render the EGA symbol `prof` rather than English `depth` because the
    symbol is the entry in the notation index; the term "depth" appears in English running prose.
- Regular system of parameters: `(t_1, …, t_n)` with `(t_1, …, t_n)·A = 𝔪` and `n = dim(A)`.
...
- `M`-regular sequence (`suite M-régulière`): a sequence `(f_1, …, f_n)` such that each `f_i` is a non-zero-divisor on
    `M / (f_1, …, f_{i-1})M`.
- `ℱ`-regular sequence (`suite ℱ-régulière`): the sheaf-of-modules version; rendered with the script `ℱ`.
...
- Constructible function/set: inherited from `(0_III, 9.1.x)` rendering.
...
- Regular immersion of codimension `n`: EGA's "immersion régulière"; we render literally. Definition at `(0_IV, 15.x)`.
- Transversally regular immersion: EGA's "immersion transversalement régulière"; locked at §IV.19.1.
- Sheaf of meromorphic functions: `𝓜_X`, `𝓜_X^×`. (Distinct from `ℳ_X`, which EGA uses for ideal sheaves; check
    context.)
