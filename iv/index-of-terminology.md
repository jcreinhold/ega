...
Alphabetized by the English head term, merging the four French part-indices of EGA IV into a single English index;
section numbers refer to the defining occurrence, with the chapter prefix (`0_IV`, `IV`) included to disambiguate
Chapter 0 (continued) entries from Chapter IV proper.
...
- Algebra, finite and étale (over `𝒪_X`): IV.18.2.3.
...
- Cohen-Macaulay `𝒪_X`-Module at a point, Cohen-Macaulay `𝒪_X`-Module: IV.5.7.1.
...
- `D_B(L)`: see Extension, trivial type.
...
- Defect, `k_0`-admissibility, of an extension: 0_IV.21.6.1.
- Defined on a field (subset of a prescheme): IV.4.8.4.
- Defined on a field (sub-`𝒪_X`-Module): IV.4.8.4.
...
- Depth of a Module at a point, of an `𝒪_X`-Module at a point: IV.5.7.1.
- Depth of a module on a Noetherian local ring: 0_IV.16.4.5.
- Depth, rectified, see Rectified depth.
- Depth, `T`-, of a coherent `𝒪_X`-Module at a point: IV.19.9.1.
- Derivation, `A`-: 0_IV.20.1.2.
- Derivation, `S`-, `(X/S)`-, `f`-: IV.16.5.1.
- Differential of a section of `𝒫^1_{B/A}`: IV.16.3.6.
...
- Equidimensional at a point (`𝒪_X`-Module): IV.5.1.12.
...
- Extension, `k_0`-admissible, of a field for an extension: 0_IV.21.6.1.
...
- `f^cons`, see Constructible topology.
- Faithfully flat Module relative to a morphism (`𝒪_X`-): IV.2.2.4.
...
- Field, `k_0`-admissible, see Extension, `k_0`-admissible.
...
- Geometrically integral `𝒪_X`-Module over a field: IV.4.6.19.
...
- Geometrically normal, reduced, regular ring, ring possessing the property `(R_k)` geometric: IV.6.7.6.
- Geometrically pointwise integral `𝒪_X`-Module over a field: IV.4.6.22.
- Geometrically pointwise integral prescheme (over a field): IV.4.6.9.
- Geometrically reduced (or separable) `𝒪_X`-Module over a field: IV.4.6.17.
- Geometrically reduced (or separable) `𝒪_X`-Module over a field at a point: IV.4.6.22.
...
- Homomorphism, regular, of `𝒪_X`-Modules: IV.19.4.11.
...
- Intersection complete at a point relative to `S`, intersection complete relative to `S` (for a flat prescheme locally
    of finite presentation): IV.19.3.6.
...
- Irredundant `𝒪_X`-Module: IV.3.2.4.
...
- Module, `ℱ`-fidelily flat (relative to a morphism), see Faithfully flat Module.
- Module, geometrically integral over a field, see Geometrically integral `𝒪_X`-Module.
- Module, geometrically pointwise integral over a field, see Geometrically pointwise integral `𝒪_X`-Module.
- Module, geometrically reduced (or separable) over a field, see Geometrically reduced `𝒪_X`-Module.
- Module, `A`-, possessing the property `(S_k)`: IV.5.7.2.
- Module, imperfection: 0_IV.20.6.1.
- Module, intègre, see `𝒪_X`-Module, integral.
- Module, irredundant, see Irredundant `𝒪_X`-Module.
...
- Module, torsion-free, relative to `S` (`𝒪_X`-): IV.20.6.2.
- Module, torsion-free (`𝒪_X`-): IV.20.1.5.
...
- Morphism, codepth `≤ n`, at a point: IV.6.8.1.
...
- Morphism, possessing the property `(R_k)` at a point, possessing the property `(R_k)`: IV.6.8.1.
- Morphism, possessing the property `(S_k)` at a point, possessing the property `(S_k)`: IV.6.8.1.
...
- Multiplicity, radicial, of a point for an `𝒪_X`-Module (`X` prescheme over a field): IV.4.7.5.
- Multiplicity, radicial, of a prime cycle maximal associated to an `𝒪_X`-Module (`X` prescheme over a field): IV.4.7.8.
...
- Multiplicity, total, of a point (of a prime cycle maximal associated) for an `𝒪_X`-Module (`X` prescheme over a
    field): IV.4.7.12.
...
- Normally flat `𝒪_Y`-Module along a sub-prescheme: IV.6.10.1.
...
- Order of a meromorphic function at a point `x ∈ X^{(1)}`: IV.21.6.7.
- `𝒪_X`-Module, see Module.
- `𝒪_X`-Module, integral, integral at a point: IV.3.2.4.
- `𝒪_X`-Module, irredundant: IV.3.2.4.
- `𝒪_X`-Module, normally flat along a sub-prescheme: IV.6.10.1.
- `𝒪_X`-Module, possessing the property `(S_k)`, possessing the property `(S_k)` at a point: IV.5.7.2.
- `𝒪_X`-Module, reduced, reduced at a point: IV.3.2.2.
- `𝒪_X`-Module, `Z`-closed, `Z`-pure: IV.5.9.9.
- `𝒪_X`-Module, `Z`-closed, `Z`-pure at a point: IV.5.9.9.
...
- Part, principal, of order `n`, of order infinity, of a section of `𝒪_X`: IV.16.3.6.
...
- Prescheme, possessing the property `(R_k)`, possessing the property `(R_k)` at a point: IV.5.8.2.
- Prescheme, possessing the property `(R_k)` geometric at a point, possessing the property `(R_k)` geometric (over a
    field): IV.6.7.6.
- Prescheme, possessing the property `(S_k)`, possessing the property `(S_k)` at a point: IV.5.7.2.
...
- Property `(R_k)`: see Prescheme, possessing the property `(R_k)`.
- Property `(R_k)` geometric: see Prescheme, possessing the property `(R_k)` geometric.
- Property `(S_k)`: see `𝒪_X`-Module, possessing the property `(S_k)`.
- Property `𝐏` geometric: IV.7.3.6.
- Property `𝐏` of the first type (of the second type) associated to a property `𝐑` of Noetherian local rings: IV.7.3.10.
...
- Reduced `𝒪_X`-Module, reduced module over a ring: IV.3.2.2.
- Regular at a point, regular morphism, see Morphism, regular.
- Regular element, see Element, `M`-regular.
- Regular homomorphism of `𝒪_X`-Modules: IV.19.4.11.
...
- Ring, geometrically normal, geometrically reduced, geometrically regular, possessing property `(R_k)` geometric:
    IV.6.7.6.
...
- Ring, `𝐏`-: IV.7.3.13.
- Ring, possessing property `(R_k)`: IV.5.8.2.
...
- Section, meromorphic, of an `𝒪_X`-Module: IV.20.1.3.
- Section, meromorphic, of an `𝒪_X`-Module defined at a point: IV.20.1.7.
- Section, meromorphic, of an `𝒪_X`-Module inversible regular relative to `S`: IV.20.6.5.
- Section, meromorphic, of an `𝒪_X`-Module relative to `S`: IV.20.6.2.
- Section, meromorphic, regular, of an invertible `𝒪_X`-Module: IV.20.1.8.
- Section, `ℱ`-regular, see Family, `ℱ`-regular section.
...
- Sheaf of modules of fractions with denominators in `𝒮`: IV.20.1.2.
- Sheaf of `S`-derivations of `𝒪_X`: IV.16.5.7.
- Sheaf of `S`-derivations of `𝒪_X` into `ℱ`: IV.16.5.4.
...
- Sheaf of rings of fractions with denominators in `𝒮`: IV.20.1.1.
...
- Sub-`𝒪_X`-Module, defined on a field: IV.4.8.4.
- Sub-`𝒪_X`-Module, primary, of an `𝒪_X`-Module: IV.3.2.4.
...
- Suite `ℱ`-quasi-regular: 0_IV.15.2.2.
- Suite `ℱ`-regular, see Sequence, `ℱ`-regular.
...
- Symmetry, canonical, of `X ×_S X`: IV.16.3.3.
- Symmetry, canonical, of `𝒫^•_{X/S}`, of `Ω^•_{X/S}`: IV.16.3.4.
...
- Torsion-free `𝒪_X`-Module, see Module, torsion-free.
