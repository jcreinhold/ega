...
$(0_{IV}, \cdots)$, $(M, \cdots)$, $(G, \cdots)$, $(T, \cdots)$, $(FAC, \cdots)$, `(Bourbaki, Alg. comm., …)`, $(Nagata,
\cdots)$, $(Zariski-Samuel, \cdots)$. Re-read those sections in $docs/books/ega/iv/conventions.md$ before extending the
...
- $p\hat{e}le m\hat{e}le Fr$ → "pell-mell", "jumbled together", or restructure the sentence.
...
| `OX`, `OY`                   | $\mathcal{O}_{X}$, $\mathcal{O}_{Y}$           | Script 𝒪.                                                                                                |
| $d2X/Y \phi$                 | $d^{2}_{X/Y} \phi$                             |                                                                                                          |
| $PX/Y2$                      | $\mathcal{P}^{2}_{X/Y}$                        | Script 𝒫 for principal parts (cf. EGA IV §0_IV.20).                                                      |
