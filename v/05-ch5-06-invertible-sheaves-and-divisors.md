...
torsors under the projective group, we postpone the study of such schemes — and of their connection with the notion
of linear system — to Chapter VI.
...
invertible modules over `P` reduces to that over `f⁻¹(S^i)` for `i = 0, 1, 2`. The case `i = 2` follows from (1.1);
on the other hand, `f⁻¹(S^1)` is `S^1`-isomorphic to `S^1`, so its Picard group is just `Pic(S^1)`; and finally
`f⁻¹(S^0)` is empty, so its Picard group is zero.*
...
`L` over `P` is isomorphic to a module of the form `f*(M)(n)`, where `n ∈ ℤ` and `M` is an invertible module over
...
`Pic(S) → Pic(P)` (deduced from `f : P → S`) and `ℤ → Pic(P)` (determined by the class `[𝒪_P(1)] ∈ Pic(P)`). We deduce
...
If we drop the assumption that the rank of `E` is `≥ 2`, it follows from (1.2) and (1.4) that the homomorphism `(*)`
is still surjective when `S` is connected, but not necessarily injective: its kernel is isomorphic to `ℤ` if `E` is of
rank `1`, and to `Pic(S) × ℤ` if `E` is of rank zero.

#### Proof of (1.1).

We start from *uniqueness*. Suppose first `S = Spec(K)` is the spectrum of a field. Then `𝒪_P(n)` is not isomorphic
to `𝒪_P(m)` for `n ≠ m`; equivalently, `𝒪_P(m − n)` is not isomorphic to `𝒪_P` unless `m − n = 0`. Indeed, this follows
...
`L(−n) ⥲ f*(M)`, which yields an isomorphism of the right-hand side of `(**)` with `f_*(f*(M)) ≅ M ⊗ f_*(𝒪_P)`, and
since `f_*(𝒪_P) ⥲ 𝒪_S`, we deduce `(**)`.
...
*Let `E` be locally free of finite rank over `S`, `P = ℙ(E)`, `L` an invertible module over `P`, and `s ∈ S`. Then
there exists an open neighbourhood `U` of `s` and an integer `n ∈ ℤ` such that `L | f⁻¹(U)` is isomorphic to
`𝒪_P(n) | f⁻¹(U)`.*
...
`𝓛` over the graded ring `K[t_0, …, t_r]`. The restriction `L'` of `L` to the punctured affine space
`𝔼^{r+1}_K − \{0\}` is the inverse image of `L` under the canonical projection morphism
`q : 𝔼^{r+1}_K − \{0\} → ℙ^r_K`, and is therefore an invertible module. Let
`i : 𝔼^{r+1}_K − \{0\} ↪ 𝔼^{r+1}_K` be the canonical immersion. The affine ring `K[t_0, …, t_r]` of `𝔼^{r+1}_K` is
factorial; *a fortiori* its localization at the origin is factorial, and that ring has dimension `≥ 2`. It follows
that `i_*(L')` is an invertible module, corresponding to an invertible graded module `𝓜` over `K[t_0, …, t_r]`.[^v-6-3]
Moreover `𝓜 = Γ(𝔼^{r+1}_K − \{0\}, q*(𝓛))` is graded in a natural fashion, and the homomorphism `𝓛 → 𝓜` is an
isomorphism at every point of `𝔼^{r+1}_K` distinct from the origin.

Replacing `𝓛` by `𝓜`, we are reduced to the case of an invertible `𝓛`. Since `K[t_0, …, t_r]` is factorial, `𝓛` is
then free of rank one when we ignore its grading. A standard lemma — without doubt available in Bourbaki — implies
that it is also free of rank one as a graded module, which means that the associated `𝒪_P`-module is isomorphic to
some `𝒪_P(n)`. From the editorial point of view, it would be clumsy to begin by considering an `𝓛` of finite type;
one should rather begin by introducing directly `𝓛 = Γ(𝔼^{r+1}_K − \{0\}, q*(L))`, define the module `𝓛 = q_*(𝓛)`, and
remark (by Chapter II) that `𝓛` is a graded module that determines `L`; then one shows that `𝓛` is free of rank `1`
...
Let `ℤ(S)` denote the set of locally constant functions `S → ℤ`. We define an evident homomorphism
`ℤ(S) → Pic(P)`: an element `n ∈ ℤ(S)` corresponds to a partition `(S_n)_{n ∈ ℤ}` of `S` into disjoint open subsets
(some possibly empty), and to such a partition we associate the invertible module `𝒪_P(n)` whose restriction to
`f⁻¹(S_n)` is `𝒪_P(n)`. We thus obtain a variant
...
bijection. (Indeed, when `S ≠ ∅`, the canonical map `ℤ → ℤ(S)` sending `n` to the constant function of value `n` is
injective; it is bijective if and only if `S` is connected. So we recover (1.4) formally, and the statement above is
the most general form.)

We also remark — and this is a remark worth making — that in the language of the Picard scheme (which will be
introduced in [Chapter V.1.1](#)[^v-6-4]), the preceding statement reads simply: *under the conditions of (1.1), the
canonical homomorphism `ℤ_S → Pic_{P/S}` of constant group schemes `ℤ` over `S` into the relative Picard scheme,
deduced from the section of the latter defined by `𝒪_P(1)`, is an isomorphism.*
...
if `L` is isomorphic to `𝒪_P(n)`; if `dim P ≥ 1`, this determines `n` in terms of `L`, but if `dim P ≤ 0` (`P` empty or
...
of `P` are non-empty (`E` everywhere of rank `≥ 1`), then `M` is determined up to a unique isomorphism by `L`, again
by (1.1) and (1.2).
...
> fibres of `P` are non-empty (i.e. the rank of `E` is `≥ 1`), then the partition of `S` is also uniquely determined
...
the `S`-morphisms `g : P → P^1`: these are defined by an invertible module `L = g*(𝒪_{P^1}(1))` over `P` together with
a homomorphism `E^1 → f_*(L)` (such that the associated homomorphism `f*(E^1) → L` is surjective), modulo an
isomorphism of `L`. We say that `g : P → P^1` is *of degree `n`* if `L = g*(𝒪_{P^1}(1))` is of degree `n`. It is
...
`K` with `dim P ≥ 1`, `𝒪_P(n)` is generated by its sections only when `n ≥ 0`). One could of course restrict to the
case where the fibres of `P` have dimension `≥ 1`, by proceeding as in (1.2). Since
...
homomorphism `f*(E^1) → f*(M)(n)` is an epimorphism. The morphism `g` then determines a first global invariant over
`S`, namely the class `[M] ∈ Pic(S)`; with `M` fixed, the corresponding `g` correspond to a certain subset of the
quotient set `Hom(E^1, M ⊗ Sym^n(E)) / Γ(S, 𝒪_S)^×`, the passage to the quotient by `Γ(S, 𝒪_S)^×` corresponding to
"module isomorphism" in the description of `g : P → P^1` via invertible modules.
...
of `M`. We find exactly the morphisms `g : P → P^1` of the form induced by a section `h` of `P^1` over `S` (namely
...
immersion deduced from the epimorphism `E^1 → E ⊗ M`. We call the homomorphisms `P → P^1` of this form **linear**;
the morphisms `g : P → P^1` of degree `1` are then exactly the linear ones.
...
*Let `S` be a prescheme, and let `P = ℙ(E)`, `P^1 = ℙ(E^1)` be two projective fibrations over `S` defined by `E`,
`E^1` locally free of finite type. Then every `S`-isomorphism `g : P → P^1` is definable as a composition*
...
is the isomorphism deduced from an isomorphism `u : E^1 ⥲ E ⊗ M`. Provided the fibres of `P` are non-empty
(resp. of dimension `≥ 1`), `M` (resp. the pair `(M, u)`) is determined up to a unique isomorphism by `g`.*

By the foregoing considerations we are reduced to showing that `g` is of degree one. This in turn reduces (by passing
to fibres) to the case where `S` is the spectrum of a field and (without loss of generality) `dim P ≥ 1`. But then
`𝒪_P(1)` is intrinsically characterized — independently of the way `P` is realized as a projective fibration — as the
ample generator of `Pic(P)` (between the two generators `𝒪_P(1)` and `𝒪_P(−1)`); consequently, if `g : P → P^1` is an
...
*Let `S` be a prescheme, `P = ℙ(E)` a projective fibration over `S` defined by `E` locally free of finite type, and
`u` an automorphism of `P`. Then `u` is determined, in a neighbourhood `U` of every point `s ∈ S`, by an automorphism
`ũ` of `E | U`, the latter being well defined by `u` modulo multiplication by an element of `Γ(U, 𝒪_U)^×`.*
...
`GL(E)` by its centre `𝔾_m`. This group-prescheme is called the **prescheme of projective groups**, or simply the
**projective group**, defined by `E`, and is denoted `PGL(E)`. If `E` is free, `E ≅ 𝒪_S^{r+1}`, then `PGL(E)` is just
the group-prescheme `PGL(r)_S` deduced by base change `S → Spec(ℤ)` from the analogous group-scheme `PGL(r)` over
`Spec(ℤ)`, called the **absolute projective group**.*
...
data as an invertible module `L` over `P` together with a *transversally regular* section `φ` of `L`. By (1.1),
ignoring a possible partition of `S` if `S` is not connected, `L` is isomorphic to `M ⊗ 𝒪_P(n)` for some invertible
...
`φ` is transversally regular (i.e. regular on each fibre) if and only if `ψ(s) ≠ 0` for every `s ∈ S`; equivalently,
the homomorphism `M^∨ → Sym^n(E)` defined by `ψ` is "universally injective" (i.e. locally an isomorphism onto a direct
...
invertible submodules of `Sym^n(E)` which are locally direct factors `M^∨`). If `D` and `M` correspond, then `𝒪_P(D)`
is canonically isomorphic to `M ⊗ 𝒪_P(n)`, and the canonical section `s_D` is identified, under this isomorphism,
...
*The functor `Div^+_{P/S}` is canonically representable by the sum-prescheme of the `ℙ(Sym^n(E)^∨)` for `n ∈ ℕ`.*[^v-6-8]
...
sections exist locally over `S` in any case), the system defines, for each `i`, an `S`-morphism `g_i : P_i → P`; we
then have
...
determination of `Div^+(P/S)` reduces to that of `Div^n(P/S)` for given multidegree `n ∈ ℤ^I`. This gives an
isomorphism `L = 𝒪_P(D) ≅ M ⊗_{𝒪_S} 𝒪_P(n)`, and by (II, §2)
...
`(⨂_i Sym^{n_i}(E_i))^∨` (equivalently, of invertible submodules locally direct factors `M^∨` of
`⨂_i Sym^{n_i}(E_i)`). If `D` and `M` correspond, then `𝒪_P(D)` is canonically isomorphic to `M ⊗ 𝒪_P(n)`, and `s_D`
...
— indeed from the "discrete" character of the Picard prescheme `Pic_{P/S}`. We can abstract from the foregoing
reasoning to a relative representability statement (relative to `Pic`).*

To make this precise, take a proper flat morphism `f : X → S` of finite presentation and an invertible module `L`
over `X`. We propose to determine the subset `Div^L(X/S)` of `Div^+(X/S)` formed by those positive relative divisors
`D` such that `𝒪_X(D)` is isomorphic to a module of the form `L ⊗_{𝒪_X} f*(M)` for some invertible module `M` over
...
*Let `f : X → S` be a proper morphism of finite presentation satisfying the above condition `(***)` universally, and
...
retrocompact open sub-prescheme of the projective fibration `ℙ(Q)`. If, in addition, the fibres of `X/S` are
geometrically integral, then `Div^L_{X/S}` is representable by `ℙ(Q)` itself.*

The last assertion follows immediately from the construction above. For the general case we have already observed
that there is a monomorphism `Div^L_{X/S} → ℙ(Q)`, and we are reduced to showing that this is a retrocompact open
immersion. So we take a section of `ℙ(Q)` over `S` — i.e. an invertible quotient module `M` of `Q`, giving a section
`φ` of `L ⊗ f*(M)` non-vanishing on every fibre — and we must show that the subfunctor of the final functor
`S' ↦ Sch_{/S}` consisting of those `S' → S` for which `φ_{S'}` is transversally regular is representable by a
...
**(3.1).** Let `D` be a family of positive divisors over `X/S` parametrized by `T`. A point `x ∈ X` is called a
**fixed point** of this family if `pr_1^{−1}(x) ⊂ D` set-theoretically, so that the set of non-fixed points is the
complement of `pr_1(X ×_S T − D)`. Consequently, if `T → S` is universally open (e.g. flat locally of finite
...
If `Z` is closed, then `X − Z` is the largest open subset of `X` such that the induced family of divisors of
`(X − Z)/S` parametrized by `T` is without fixed points. If the family `D` is without fixed points and `T` is flat
and locally of finite presentation over `S` with geometrically irreducible fibres satisfying `(S_1)`, then `D` is also
a relative divisor with respect to `X` (for `pr_1 : X ×_S T → X`). Indeed, `D` is defined locally at a point
`z ∈ supp D` by one equation `φ = 0`, and the equation `φ_x` induced on the fibre `T ⊗ k(x)` is non-nilpotent at `z`
(otherwise, `pr_1^{−1}(x) ∩ D = V(φ_x)` would contain a neighbourhood of `z` in `T ⊗ k(x)`, hence the whole of
`T ⊗ k(x)` since this fibre is irreducible — meaning `x` would be a fixed point, which it is not). Since `T ⊗ k(x)`
is irreducible and satisfies `(S_1)`, it follows that `φ_x` is `𝒪_{T ⊗ k(x)}`-regular at `z`. We obtain therefore a
...
`T/S` parametrized by `X − Z`,[^v-6-10] i.e. a morphism `X − Z → Div_{T/S}`, by replacing `X` by `X − Z` in the
preceding construction. The argument shows in fact that `X − Z` is exactly the greatest open subset `U` of `X` such
that `D | U ×_S T` is a relative divisor of `U ×_S T` with respect to `U` — equivalently, such that its "symmetric
image" `ᵗD` is a family of divisors of `T/S` parametrized by `U/S`.

We remark that if `X` and `T` are both flat locally of finite presentation over `S`, with geometrically irreducible
fibres satisfying `(S_1)`, then the symmetry `D ↦ ᵗD` gives a one-to-one correspondence between families of divisors
...
points in the extended sense**: by such a fixed point of `D` we mean an `x ∈ X` such that `D` is not a relative
divisor with respect to `X` at all the points of `pr_1^{−1}(x)`. If `W` is the open subset of `X ×_S T` where `D` is a
relative divisor with respect to `X`, then the set of fixed points in the extended sense of `D` equals
`pr_1(X ×_S T − W)`; since `T → S` is proper, this is a closed subset `Z'` of `X`. In every case we obtain a family of
...
point (resp. fixed point in the extended sense) means that `x ∈ supp D_t` for every `t ∈ T(k)` (resp. that there
exists a prime cycle `T'` associated to `T` such that `x ∈ supp D_t` for every `t ∈ T'(k)`).
...
When `T` is a projective fibration `Q = ℙ(F)`, the functor `Div^+_{Q/S}` is representable by the sum-prescheme of
the `ℙ(Sym^n(F^∨))` (which we denote `ℙ(n)`); we find then a morphism

```text
  X − Z ⟶ ⨆_n ℙ(n).
```

We say that the family of divisors `D` is **of degree `n`** if the preceding morphism factors through `ℙ(n)`. If
`X ≠ ∅` (so `X − Z ≠ ∅`), the integer `n` is well defined by `D`. To define this notion of degree, we strictly need
...
over `X/S` parametrized by `Q` which is of degree one, i.e. defining `f : X − Z → P`. To such a linear system, when
the fibres of `P^∨` are non-empty,[^v-6-12] is associated a *rational map* (better: a "pseudo-morphism relative to
`S`") `X ⤏ P`.

By the very construction, `D | (X − Z) ×_S P^∨` is the inverse image, under `f × id_{P^∨}`, of the canonical *incidence
divisor* `H` over `P ×_S P^∨`. Hence the knowledge of `f : X − Z → P` allows us to reconstruct, at least, the family
of divisors of `(X − Z)/S` induced by `D`. So if the family is without fixed points it is determined by the associated
morphism `f : X → P`. We obtain a one-to-one correspondence between linear systems without fixed points over `X/S`
parametrized by `P^∨` and morphisms `f : X → P` such that `(f × id)^{−1}(H)` is a relative divisor of `X ×_S P^∨`
with respect to `P^∨`. This condition can be checked fibre by fibre:
...
by `P^∨` and morphisms `f : X → P` having the following property: for every `s ∈ S`, denoting by `\overline{k(s)}` an
...
trivial case is `P^∨ = S` of relative dimension zero: a linear system of divisors of `X/S` parametrized by `S` is
just a relative Cartier divisor `D` over `X` relative to `S`, the associated morphism is the projection
`X − D → S`, and the knowledge of this morphism (including its domain of definition) does not determine `D`.
...
is **without fixed components** if it has no fixed component, i.e. `codim(Z, X) ≥ 2`. This terminology extends
...
*Suppose `X → S` is flat and locally of finite presentation, with fibres satisfying `(S_2)`. Let `D` be a linear
system of divisors without fixed components over `X/S` parametrized by `P^∨`. Then `D` is uniquely determined by the
...
We must prove that if `D'` is another linear system of divisors without fixed components parametrized by `P^∨`,
defining `f' : X − Z' → P`, and if `f` and `f'` agree on an open subset `U ⊂ (X − Z) ∩ (X − Z')` schematically dense
relative to `S`, then `D = D'`. Since `P` is separated over `S`, we may take `U = (X − Z) ∩ (X − Z') = X − Z''`, where
...
`𝒥 | X − Z'' → 𝒪_X | X − Z''`. Since `𝒥` is invertible it is flat over `S`, and for `x ∈ Z''`,
`prof_x 𝒥_{(s)} ≥ 2`. It is therefore enough to prove:
...
implies, by `(IV, §6)`, that `prof_x 𝓕 ≥ prof_x 𝓕_s` for every `x ∈ X` over `s ∈ S`, so `prof_x 𝓕 ≥ 1` (resp. `≥ 2`)
for `x ∈ T`. We conclude by `(IV, §5)`.[^v-6-14]
...
`U → P` is associated to a linear system of divisors `D''` over `U/S` parametrized by `P^∨`, and `D'' | (X − Z) ×_S P^∨
= D | (X − Z) ×_S P^∨`. Applying the uniqueness result to `D''` and `D | U ×_S P^∨`, the two are equal, so
`D | U ×_S P^∨` has no fixed points: `Z ∩ U = ∅`, hence `U = X − Z`. q.e.d.
...
*(a) This map is injective, and for `D ∈ Φ` the set of fixed points `Z` is just the complement `X − U` of the domain
of definition of `f_D`.*

*(b) Let `f ∈ 𝔘`, and let `U` be an open subset over which `f` is defined and such that `z ∈ X − U ⟹ prof_z 𝒪_{X_s,
z} ≥ 2` (for instance `U = U(f)`). In order that `f` give rise to some `D ∈ Φ`, it is necessary and sufficient that,
setting `L_U = f_U^*(𝒪_P(1))` (with `f_U : U → P` the morphism induced by `f`), the module `i_*(L_U)` over `X` is
invertible (where `i : U ↪ X` is the canonical immersion).*

*If the fibres of `X` over `S` satisfy `(S_2)` — for instance if they are normal, or even just geometrically normal —
then the depth condition on a closed subset `Z` of `X` in the proposition means simply that for every `s ∈ S`, `Z_s`
is of codimension `≥ 2` in `X_s`; `Φ` is therefore exactly the set of linear systems without fixed components.
Moreover, if `S = Spec(k)` and `X` is normal, then for every rational map `f : X ⤏ P` the domain of definition
`U(f)` satisfies `codim(X − U(f), X) ≥ 2` (II.7), so `𝔘` is the set of all rational maps `X ⤏ P`.*
...
`D' = D | U ×_S P^∨` and `N' = N | U ×_S P^∨`, where `N` is the invertible module over `X ×_S P^∨` defined by `D`.
It follows from Lemma (3.3), applied to `P ×_S P^∨ → P`, that `i_*(N') ≅ N`; hence `i'_*(N')` is invertible.

Conversely, if this condition is satisfied, we show that `f` gives rise to a `D ∈ Φ`, which evidently amounts to
saying that `D'` extends to a relative divisor of `X ×_S P^∨` with respect to `P^∨`. It suffices to show that it
extends to a divisor `D` over `X ×_S P^∨` (it will then automatically be a relative divisor with base `P^∨`, since
`U` contains elements associated to `𝒪_{X_s}` for every `s ∈ S`, and that property is stable under base change — in
particular under `S' = P^∨ → S`). But by Lemma (3.3) again, `D'` extends to a divisor `D` if and only if `D'` extends
...
*Suppose `g : X → S` is flat and locally of finite presentation. Let `T` be a closed subset of `X` such that `x ∈ T
⟹ prof_x 𝒪_{X_s} ≥ 2` (where `s = g(x)`), and set `U = X − T`, `i : U ↪ X`. For every locally free module `L` of
...
*Suppose the local rings of `X` are factorial (for example, `X` regular). Then the map `Φ → 𝔘` of (3.4) is bijective.
In particular, if `X` is a regular prescheme locally of finite type over a field `k`, and `P` is a projective
fibration over `k`, then there is a one-to-one correspondence between the set `Φ` of linear systems of divisors
without fixed components over `X` parametrized by `P^∨`, and the set `𝔘` of rational maps `X ⤏ P` whose image
...
1. *(regularity fibre by fibre)* the sections `φ_t` (`t ∈ P^∨`) induced by `φ` on the fibres of `X ×_S P^∨` over
    `P^∨` are regular (which entails that `φ` is regular); and
...
(where `(−1)` denotes tensoring with `𝒪_{P^∨}(−1)` over `𝒪_S`). To give `φ` is then to give a section of
`L ⊗ 𝒪_P(1)`, i.e. a section of
...
by `E` locally free of finite type, everywhere `≠ 0` (so that `P` has non-empty fibres), and `P^∨ = ℙ(E^∨)`. Then
there is a bijection between the set of linear systems of divisors over `X/S` parametrized by `P^∨` and the set of
pairs `(L, u)` (up to isomorphism), where `L` is an invertible module over `X` and `u : E → g_*(L)` is a homomorphism
such that for every `s ∈ S` and every point `t` of `E(s) ⊗_{k(s)} \overline{k(s)}` (`\overline{k(s)}` an algebraic
closure of `k(s)`), the corresponding section `u(t)` of `L_s ⊗ \overline{k(s)}` over `X_s ⊗ \overline{k(s)}` is
regular.*

<!-- original page 15 -->

If the fibres of `X` are geometrically integral, the condition on `u` simplifies: for every `s ∈ S`,
`u(s) : E(s) → H^0(X_s, L_s)` is injective. For convenience of reference, we recall the construction of the divisor
`D` in terms of `(L, u)`: it is the divisor of the evident section `φ` of `L ⊗_{𝒪_S} 𝒪_{P^∨}(1)` defined by `u`.
...
invertible module over `X` and `P = ℙ(E)` a projective fibration over `S` as in (4.1). There exists a finitely
presented module `Q` over `S` and an isomorphism of functors of the quasi-coherent `𝒪_S`-module `F`:*
...
(4.1) correspond bijectively to the surjective homomorphisms `Q → E^∨` modulo multiplication by a section of
`𝒪_S^×`.*
...
`L`, by virtue of V.6.2. Consequently, a linear system parametrized by `P^∨` and associated to `L` is interpreted as
a morphism `P^∨ = ℙ(E) → ℙ(Q)`. The "linear" character of the family of divisors so defined is captured by saying that
this morphism is *linear*, i.e. is defined by a surjective morphism `Q → E^∨`. In this case `P^∨ → Div_{X/S}` is also
a monomorphism (since `P^∨ → ℙ(Q) → Div_{X/S}` is such); this is a more general fact, cf. the corollary below.
...
We should also make explicit in (4.1) that `L | X − Z` is canonically isomorphic to `f*(𝒪_P(1))` (with the notations
of V.6.3). So for a `D ∈ Φ` in the sense of (3.4), `L | X − Z` is exactly the canonical and unique extension of
...
`\overline{k(s)} ⥲ H^0(T, 𝒪_T)` (a condition automatically satisfied if `g` is proper and surjective). Then the
morphism `D : P^∨ → Div_{X/S}` is a monomorphism.*
...
and `X` an open subset of `ℙ^1_k` not containing two distinct points `a, b ∈ ℙ^1_k(k)`; then `a` and `b` define the
...
`H^0(X, 𝒪_X) ⥲ 𝒪_S`. Then for every `S'` over `k`,

```text
  g_{S', *}(𝒪_{X_{S'}}) ⥲ 𝒪_{S'}.
```

Now if `L` over `X` and `u : E → g_*(L)` are as in (4.1), and if two sections `φ`, `ψ` of `E_{S'}` everywhere
non-zero are such that `u(φ)` and `u(ψ)` have the same divisor as sections of `L_{S'}` over `X_{S'}`, then `u(ψ)` is
deduced from `u(φ)` by multiplication by an invertible section of `𝒪_{X_{S'}}`; from `(g_{S'})_*(𝒪_{X_{S'}}) ⥲
𝒪_{S'}`, this multiplier is an invertible section of `𝒪_{S'}`. So `ψ = c · φ` for `c ∈ 𝒪_{S'}^×`, meaning `φ` and
`ψ` define the same point of `P^∨` with values in `S'`. Since every point of `P^∨` with values in `S'` is defined
locally over `S'` by a section `φ` of `E_{S'}` not vanishing anywhere (cf. Chapter I), (a) follows.
...
`k`, parametrized by `P^∨ = ℙ(E)`, and let `f : X − Z → P` be the corresponding morphism (`Z` = base locus, i.e. set
of fixed points). If `r = rank_k E > 0`, there exist `r + 1` points `x_i` (`1 ≤ i ≤ r + 1`) of `X(k) − Z(k)` such
that the `f(x_i)` form a "projective base" of `P`: for every subset `J ⊂ [1, r + 1]` of cardinality `r`, the
`f(x_j)` (`j ∈ J`) are not contained in any hyperplane of `P`.*

We may suppose `Z = ∅`. Since (by (4.1)) `f(X)` is not contained in any hyperplane of `P`, we obtain at once the
existence of `r` points `x_i` (`1 ≤ i ≤ r`) such that the `f(x_i)` are projectively independent in `P`, i.e. defined
by linearly independent forms over `E`. It remains to find `x_{r+1} = x ∈ X(k)` such that `f(x)` lies in none of the
`r` hyperplanes `H_i` defined by the `(r − 1)`-tuples among the `f(x_j)`. Suppose the contrary; then by Jacobson-type
sorites,
...
is determined by the composition of its contragredient `u^∨` (on `P`) with `f : X → P`, and that the analogous
statement holds after every base change `S → Spec(k)` by an automorphism `u` of `P^∨_S`. But this follows immediately
from Lemma (4.4) and from the determination of automorphisms of `ℙ(E^∨) = ℙ(E)^∨` in V.6.1: the effect of an
...
For a noetherian base scheme `S` and a morphism of functors `h : F → G` over `S` (with `F`, `G : Sch_{/S} → Set`),
there are general criteria — to be made explicit in Chapter V — guaranteeing that if for every `s ∈ S` the morphism
...
We have two sections `u, v` of a prescheme `F` of finite type over `S` (in (a), sections of `P^∨`; in (b), sections
of the projective group `PGL(E^∨)`); we want to show they are equal. It is enough to prove this after the base change
...
where `k = A/𝔪` is the residue field, `F_0 = F ⊗_A k` the reduced fibre, and `V = 𝔪^n = 𝔪^n / 𝔪^{n+1}`. It suffices
to show that `δ = 0` using the hypothesis `h(u) = h(v)`.

The general principle is: expressing that `h(u)` and `h(v)` coincide modulo `𝔪^n`, we see that their "difference" can
be written as an element `δ'` of `Hom_k(w_0^*(Ω^1_{G_0/k}), V)` where `w_0 = h_0(u_0) = h_0(v_0)` and
`G_0 = G ×_A k`; this element is exactly the one deduced from `δ` by composition with the natural homomorphism
...
if `h_0^*` is surjective, we conclude `δ = 0`. The fact that `h_0 : F_0 → G_0` is a monomorphism (which implies that
...
vector bundle `𝒢_{w_0}` over `k` playing the role dual to `w_0^*(Ω^1_{G_0/k})`[^v-6-19] — the "tangent to `G_0` at
`w_0`" — by expressing the deviation of two points of `G` coinciding modulo `𝔪^n` as an element of `𝒢_{w_0} ⊗_k V`.
This is essentially straightforward, and is contained in the systematic developments of `(V, §26)`[^v-6-20]
...
(`𝒢_{u_0}` the dual of `u_0^*(Ω^1_{F_0/k})`) expressing the passage from `δ` to `δ'`; the injectivity follows from
the injectivity of `𝒢_{u_0} → 𝒢_{w_0}`, which comes from the fact that `h_0 : F_0 → G_0` is a monomorphism.
...
over `X/S` becomes that of a subfunctor `P^∨` of `Div_{X/S}` satisfying certain properties — namely,
...
of divisors over `X/S` parametrized by projective fibrations `P^∨`, `(P^∨)'` is induced by a unique `S`-isomorphism
`P^∨ ⥲ (P^∨)'` (compatible with `D`, `D'`).* So a class (up to isomorphism) of linear systems over `X/S` determines
its parametrizing projective fibration up to a unique isomorphism. Technically, this result will allow us — once we
have the descent theory of Chapter V (not yet written; the numbering is of only historical interest) — to perform
...
*(a) If `D'` is another linear system of divisors over `X` parametrized by `(P^∨)'`, then `D` and `D'` are isomorphic
if and only if `Ens(D) = Ens(D')`.*

*(b) Suppose `k` is algebraically closed, or `X` is geometrically integral. In order that a set `Δ` of positive
Cartier divisors over `X` should be of the form `Ens(D)`, it is necessary and sufficient that there exist a
`k`-subspace `E` of the vector space of meromorphic functions on `X` such that for every `φ ∈ E − \{0\}`, `φ` is
regular (i.e. `div(φ)` is defined), and `Δ` is the set of `div(φ)` for `φ ∈ E − \{0\}`.*

*(c) Let `E, E'` be two `k`-vector subspaces of the meromorphic functions on `X` satisfying the assumption of (b).
Then the sets of divisors `Δ`, `Δ'` defined by them are equal if and only if there exists a regular pseudo-function
`φ` over `X` such that*
...
[^v-6-1]: Translator's note: Blass renders this as "(`[Tr]` of the original design)". Grothendieck means: the
    ...
    `(Bourbaki Sém. 236)`, `(SGA 1)`, and the descent material now scattered through `(SGA 3)` and `(SGA 4½)`.
...
[^v-6-3]: Translator's note: Blass has `[illegible letter]` for the symbol naming the graded module produced by `i_*(L')`.
    The PDF shows this is `𝓜` (script M), which Grothendieck reuses on the next line; we write `𝓜` throughout.

[^v-6-4]: Translator's note: the reference is to a never-written section of the descent-theoretic Chapter V (originally
    planned); the modern locus is the relative Picard scheme `Pic_{X/S}` as constructed by `(Bourbaki Sém. 232/236)`
    or `(FGA)`. The present text is silent on its existence; we keep Grothendieck's forward reference verbatim.

[^v-6-5]: Translator's note: Blass writes "Marginal remark next to Remark 1.9 partly illegible `[illegible]` / `ℙ(E ⊗ E)`
    defined by the non-vanishing of the 'determinant'". The PDF (page 6) shows the marginal note ends with `ℙ(E ⊗ E^∨)`,
    not `ℙ(E ⊗ E)`; the correction is consistent with the modern description of `PGL(E)` as the open subscheme of
    `ℙ(End(E)) = ℙ(E ⊗ E^∨)` complementary to the determinant locus. We restore `E ⊗ E^∨`.
...
[^v-6-10]: Translator's note: Blass writes "in the previous definition `X` by `X − T`" with a footnote "I think
    `X − Z` `[Tr]`". The PDF confirms `X − Z`; we adopt it silently.
...
[^v-6-17]: Translator's note: Blass writes "of 20.3 (Reference hard to locate, ask AG for help)" with a footnote
    "Ask A.G., is it EGA 21.3 `[Tr]`?". The Grothendieckian reference is to §V.6.2 of the *present* volume (the
...
[^v-6-20]: Translator's note: Blass writes "par. 26 (? Infinitesimal extensions)". The reference is to the
    infinitesimal-extension calculus of the planned `(V, §26)`; the published substitute is `(SGA 3, Exposé III)` and
    the deformation-theoretic material of `(Bourbaki Sém. 232)`.

[^v-6-21]: Translator's note: Blass writes "paragraph 25 (which it is out of the question to redo here in the
    particular case)". The reference is to the infinitesimal-deformation calculus of the planned `(V, §25)`; no
...
[^v-6-22]: Translator's note: Blass writes "and also 4.3 will not be used gain in Ch. IV except perhaps in the two
    ...
    very likely]`". The editorial note records the V↔IV renumbering ambiguity; we render the sentence in V form and
