...
- **canonical incidence divisor $H$** (on $P \times_{S} P^{\vee}$) — `(V, 6.3.1)`; see also **incidence prescheme**
...
- **conic projection of $X$ relative to $f$ with centre $C$** ($p^{X}_{C}$) — `(V, 5.14)`
- **conic projection with centre $C$** ($p_{C}$) — `(V, 5.14)`
- **conormal module $\nu^{\vee}_{X/P}$** — `(V, 5.8.6)`
...
- **determinant** (Grothendieck's proposed renaming of the ordinary discriminant of a quadratic form) — `(V, 2.16.11)`
  (Grothendieck note)
...
- **Grassmannian $Grass_{n}(\mathcal{E})$** — `(V, 5.11)`
...
- **linear sections of $X$ over $\mathbb{P}$ by linear subvarieties of codimension $m$** ($Y^{(m)}_{\xi}$) — `(V, 5.12)`
...
- **multiprojective fibration $P = \prod_{i} P_{i}$** — `(V, 6.2.5)`
...
- **orthogonal-group fibration** (the principal homogeneous fibration $Isom(E_{0}, E) \to \mathcal{Q}(E)*$) —
  `(V, 2.16.12)`
...
- **partial degree of an invertible module** with respect to the factor $P_{i}$ — `(V, 6.2.6)`
- **pencils of hyperplane sections** — `(V, 5.10)`; see also **linear pencil**
- **positive relative divisor of degree $n$** ($\operatorname{Div}^{n}(P/S)$) — `(V, 6.2.2)`
- **positive relative divisor of multidegree $n$** ($\operatorname{Div}^{n}(P/S)$, multiprojective case) — `(V, 6.2.8)`
- **prescheme of projective groups** (= projective group, = $PGL(E)$) — `(V, 6.1.9)`
- **primary extension of $k$** — `(V, 5.3.1.1)`
- **principal homogeneous fibration with group $O(n)_{S}$** (associated to a smooth quadratic form) — `(V, 2.16.12)`
...
- **quadratic form on $E^{\vee}$** (alternative interpretation of $Q \in Sym^{2}(E)$) — `(V, 2.16.1)`
- **quadric** (non-singular, hyperplane sections example) — `(V, 5.8.17)`, `(V, 5.9.1)`

## R

- **ramification subprescheme $Ram(V'/Y)$** — `(V, 1.7)`
- **rational map relative to $S$** — `(V, 6.3)`
- **regular homomorphism $F_{X}(-1) \to \mathcal{O}_{X}$** (cuts out a regular sequence) — `(V, 5.10.2)`
- **regular immersion** (deduced from a regular homomorphism) — `(V, 5.10.2)`
- **regular morphism** — `(V, 5.2.8)`
- **relative divisor over $P/S$** ($\operatorname{Div}(P/S)$) — `(V, 5.1.1)`
...
- **Seidenberg-type theorem** (openness of $(S_{k})$ / normality loci) — `(V, 5.5)`
...
- **singular zero of $\phi$ relative to $Y$** — `(V, 1.4)`
- **singular zero (root) of a section $\phi$** — `(V, 1.1)`
- **smooth form** (on a locally free module of finite type) — `(V, 2.15.1)`
- **smooth quadratic form** — `(V, 2.15.1)`, §V.2.16 (chapter title)
- **standard quadratic form $Q_{n}$ on $\mathbb{Z}^{n}$** — `(V, 2.16.4)`
...
- **supersingular zero of $\phi$ relative to $Y$** — `(V, 1.4)`
- **supersingular zero (of a section $\phi$)** — `(V, 1.1)`

## T

- **tangent hyperplane (to $X$ at $x$)** — `(V, 5.8.6)`, `(V, 5.8.7)`
- **tangent sheaf $\mathcal{G}_{X/Y}$** (kernel of the augmentation $\mathcal{P}^{1}_{X/Y} \to \mathcal{O}$) —
  `(V, 1.5)`, footnote $[{}^{v}-1-3]$
- **transversally regular section** (of an invertible module) — `(V, 6.2.1)`

## U

- **ultra-singular zero $V(\phi)_{ult sing}$** — `(V, 2.15.1)` (Grothendieck note)
...
- **zero set of $\phi$** ($V(\phi)$) — `(V, 1.1)`, `(V, 1.5)`
- **zero set of a section $\psi$** ($V(\psi)$) — `(V, 1.5)`
- **zero set singular relative to $Y$** ($V(\phi)_{sing}$) — `(V, 1.5)`, `(V, 1.7)`
- **zero set supersingular relative to $Y$** ($V(\phi)_{supsing}$) — `(V, 1.5)`, `(V, 1.7)`
