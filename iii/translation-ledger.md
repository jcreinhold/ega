...
Running French$\leftrightarrow$English term ledger for the EGA III translation. Seeded from
`zz-index-terminologique-part-1.md`, `zz-index-terminologique-part-2.md`, `zz-index-notations-part-1.md`, and
`zz-index-notations-part-2.md`. Extends the EGA II ledger at $docs/books/ega/ii/translation-ledger.md$.

## Terms inherited from EGA II

The EGA II ledger transfers unchanged: `préschéma` $\to$ prescheme, `schéma` $\to$ scheme, `morphisme structural` $\to$
structure morphism, `morphisme affine`, `morphisme propre`, `morphisme projectif`, `morphisme entier`, `morphisme fini`,
`morphisme quasi-fini`, $universellement ferm\acute{e}$, `séparé`, $tr\grave{e}s ample$, `(TF)` / `(TN)` conditions,
`éclatement` $\to$ blow-up, $c\hat{o}ne projetant$ $\to$ projecting cone, `fermeture projective` $\to$ projective
closure, `homogénéisation` $\to$ homogenization, etc.
...
| filtration co-discrète, co-séparée, etc. | co-discrete, co-separated, etc. filtration | 0_III.11.1.3 | Render the full list: discrète, exhaustive, finie, séparée $\to$ discrete, exhaustive, finite, separated. |
...
- "Préschéma formel" $\to$ "formal prescheme" throughout. EGA III §III.3–§III.4 develop proper morphisms of formal
...
- "Suites spectrales de changement de base" $\to$ base-change spectral sequences (EGA III §III.6.9). Distinct from the
  base-change spectral sequences for `Tor` and `Ext` in $(0_{III}, 12.5)$.
- "Suites spectrales d'associativité" $\to$ associativity spectral sequences (EGA III §III.6.8); the corresponding
  spectral *functor* is called "associativity spectral functor" (foncteur spectral d'associativité).
...
- For §III.6.7: EGA writes $\mathcal{H}^{-n}(f, \mathcal{P}_{\bullet})$ for hypercohomology in *negative-degree*
  ...
  \times_{S} f_{2}, -)$.
- "Edge-homomorphisme" $\to$ edge homomorphism (no hyphen in English); cf. ledger entry at $0_{III}.12.1.7$.
