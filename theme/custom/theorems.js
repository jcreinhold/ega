// AMS theorem-environment tagger (EGA build; ported from the SGA setup).
//
...
//
// EGA (like SGA) authors most statements in a SPLIT form: a label-only paragraph
...
// the comment/whitespace nodes between). The inline form — label and body in one
// paragraph (e.g. `**Proposition (7.1.5).** …`) — is still handled directly.
(function () {
  "use strict";

  // Label keyword -> AMS style bucket (see theorems.css). Plural spellings are
  // listed explicitly because the \b anchor below treats "Remarks" as a distinct
  // word from "Remark"; EGA uses the plural forms (Remarks ~123, Examples ~8,
  // Proofs 2), so they each need their own entry to be tagged.
  var KIND = {
    Theorem: "plain",
    Lemma: "plain",
    Proposition: "plain",
  ...
  // Anchored at paragraph start; \b keeps a keyword from matching a longer word
  // (so "Theorem" won't match "Theorems" — plurals we want are listed in KIND
  // above) while still matching "Proposition 0.5." (keyword followed by
  // space/number).
