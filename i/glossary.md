...
| racine d'un idéal       | radical of an ideal   | Use `radical`, not `root`; EGA writes `r(𝔞)` for what modern texts often write `√𝔞`.                       |
| anneau de fractions     | ring of fractions     | `S⁻¹A`, `A_f`, `A_𝔭`.                                                                                      |
...
| Module                  | Module (capital M)    | EGA uses capital `M` for sheaves of modules over `𝒪_X`; preserve typography.                               |
...
| topologie spectrale     | spectral topology     | EGA's name for the Zariski topology on `Spec(A)`; both names appear in EGA.                                |
...
- `il est immédiat que` is rendered as `it is immediate that`, never strengthened to `it is obvious that`.
- `on voit aussitôt que` is rendered as `one sees at once that`.
- `il est facile de voir que` is rendered as `it is easy to see that`, preserving the rhetorical force.
- `clairement` / `évidemment` is rendered as `clearly` / `evidently` — never escalated to `obviously` unless EGA uses
    the stronger word.
...
- Italics, in EGA reserved for emphasis and for first-occurrence-defines-a-term, are rendered as Markdown italics `*…*`
    for emphasis and as `**…**` (bold) for definitional first occurrences inside an unbracketed paragraph. (This follows
    the SGA-I convention.)
- Footnotes in the French source are converted to Markdown footnotes keyed by section and item: `[^N-M-K]`.
...
| ------------------ | ---------------------- | ------------------------------------------------------------------------------------------ |
| `r(𝔞)`             | `√𝔞`                   | Radical of an ideal.                                                                       |
| `𝔍`, `𝔭`, `𝔮`, `𝔪` | as such                | Fraktur letters for ideals; preserve glyphs.                                               |
| `iˢ_M`             | as such                | Canonical map `M → S⁻¹M`; preserve subscript-superscript form.                             |
| `S⁻¹M`             | `S⁻¹M` (modern same)   | Module of fractions.                                                                       |
| `Spec(A)`          | same                   | Prime spectrum.                                                                            |
| `Spf(A)`           | same                   | Formal spectrum.                                                                           |
| `Supp(M)`          | same                   | Support of a module.                                                                       |
| `M_[φ]`            | `φ_* M`                | EGA's notation for restriction of scalars; preserve.                                       |
| `lim⃗`              | `colim` or `lim_→`     | Direct/inductive limit; EGA writes `lim` with a rightward arrow above. We render as `lim⃗`. |
| `lim⃖`              | `lim` or `lim_←`       | Inverse/projective limit; rendered as `lim⃖`.                                               |
| `M ⊗_A N`          | same                   | Tensor product.                                                                            |
| `Hom_A(M, N)`      | same                   | Module of homomorphisms.                                                                   |
| `Ω¹_{X/Y}`         | same                   | Sheaf of relative differentials.                                                           |
